...
contract FundMe{
    // we can transfer the ether using three different ways:
    // transfer, send, call

        // transfer
        // msg.sender -> type = address
        // payable(msg.sender) -> type = payable address (type cast is done)
        payable(msg.sender).transfer(address(this).balance);
        // if failed then throw error
        // max 2300 gas 

        // send
        // if fail then return bool value
        bool sendSuccess = payable(msg.sender).send(address(this).balance);
        require(sendSuccess, "Send Failed");
        // max 2300 gas

        // call
        // return two variable bool and bytes
        // function is not given -> ("")
        (bool callSuccess, ) = payable(msg.sender).call{value: address(this).balance}("");
        require(callSuccess, "Call Failed");

    // receive ether for contract 
    receive() external payable {}
}


///////////////////// Transfer ether from one account to another and fund the contract /////////////////////
